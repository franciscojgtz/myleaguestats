...
<div class="primary">
  ...
  <h1>Edit League : {$league->getName()}</h1>
   
  <form class="pure-form pure-form-stacked" method='post' action='' accept-charset='UTF-8'>
  
  	 <ul class="editmenu">
			<li><a href="deleteleague.php?u_id={$userLoggedIn->getUserID()}&&l_id={$league->getLeagueID()}">Delete</a></li>
	 </ul>
    
    
    <legend>Change your league information by filling in the following form</legend>

    <p><label for="editleaguename">New League Name</label> <input type="text" name="editleaguename" id="editleaguename" value="{$leaguename}" placeholder="new league name" required/> </p>
