...
<div class="primary">
  ...
  <h1>Edit Season : {$season->getName()}</h1>
  
  <form class="pure-form pure-form-stacked" method='post' action='' accept-charset='UTF-8'>
  
  	 <ul class="editmenu">
		<li><a href="deleteseason.php?u_id={$userLoggedIn->getUserID()}&&s_id={$season->getSeasonID()}">Delete</a></li>
    </ul>
    
    <legend>Change your Season information by filling in the following form</legend>

    <p><label for="editseasonname">New Season Name</label> <input type="text" name="editseasonname" id="editseasonname" value="{$seasonname}" placeholder="season name" required/> </p>
