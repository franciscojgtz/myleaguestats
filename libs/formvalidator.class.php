...
define("E_VAL_ALNUM_S_SPECIAL_CHARS_CHECK_FAILED","Please provide an alpha-numeric and special characters input for %s");//********ADDED ON 1/31/13
...
//ADDED FROM http://www.html-form-guide.com/php-form/php-form-validation.html ON 1/30/2013
...
class FormValidator 
{
	...
	function ValidateCommand($command,$command_value,$input_value,&$default_error_message,$variable_name,$formvariables)
	{
		...
		switch($command)
		{
			...
			case 'alnum':
						{
							$bret= $this->test_datatype($input_value,"/[^A-Za-z0-9]/i");
							if(false == $bret)
							{
			...
			case 'alnum_s_special_chars':
						{
							$bret= $this->test_datatype($input_value,"/[^\p{Xwd}\(\)\.\_\- ]/u");//"/(*UTF8)[^\p{L}A-Za-z0-9\.\_\- ]/i"
							if(false == $bret)
							{
								$default_error_message = sprintf(E_VAL_ALNUM_S_SPECIAL_CHARS_CHECK_FAILED,$variable_name);
							}
							break;
						}						
            ...
            case 'numeric':
						{
							$bret= $this->test_datatype($input_value,"/[^0-9]/i");
							if(false == $bret)
							{
			...
			case 'alpha':
						{
							$bret= $this->test_datatype($input_value,"/[^A-Za-z]/i");
							if(false == $bret)
							{
